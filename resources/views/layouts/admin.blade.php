...
<div style="display:flex;height:100vh;overflow:hidden;background:#F7F8FA;font-family:'Inter',sans-serif;">
  ...
  <div style="flex:1;display:flex;flex-direction:column;overflow:hidden;min-width:0;">
    ...
    <header style="height:62px;background:#FFF;border-bottom:1px solid #E2E4EB;
      ...
      {{-- Notif --}}
      @php
      $notifPembayaran = \App\Models\Pembayaran::where('status_pembayaran','menunggu_verifikasi')
          ->with(['pendaftaran.peserta','pendaftaran.kegiatan'])
          ->orderBy('updated_at','desc')
          ->take(5)
          ->get();

      $notifPerpanjangan = \App\Models\Pembayaran::where('status_perpanjangan','menunggu')
          ->with(['pendaftaran.peserta','pendaftaran.kegiatan'])
          ->orderBy('updated_at','desc')
          ->take(5)
          ->get();

      $countPembayaran = \App\Models\Pembayaran::where('status_pembayaran','menunggu_verifikasi')->count();
      $countPerpanjangan = \App\Models\Pembayaran::where('status_perpanjangan','menunggu')->count();
      ...
      <div style="position:relative;">
        ...
        <div id="notif-drop" class="hidden" style="position:absolute;right:0;top:46px;width:320px;
            background:#FFF;border:1px solid #E2E4EB;border-radius:16px;
            box-shadow:0 16px 48px rgba(0,0,0,.15);z-index:999;overflow:hidden;" onclick="event.stopPropagation()">
          ...
          <div style="max-height:340px;overflow-y:auto;">
            ...
            <a href="{{ route('admin.pembayaran.index', ['status' => 'menunggu_verifikasi']) }}"
              ...
              <div style="flex:1;min-width:0;">
                ...
                <p style="margin:0 0 3px;font-size:11.5px;color:#4B5563;font-weight:600;">
                  {{ $np->pendaftaran?->peserta?->nama_lengkap ?? 'Peserta' }}
                </p>
            ...
            <a href="{{ route('admin.pembayaran.index') }}"
              ...
              <div style="flex:1;min-width:0;">
                ...
                <p style="margin:0 0 3px;font-size:11.5px;color:#4B5563;font-weight:600;">
                  {{ $nper->pendaftaran?->peserta?->nama_lengkap ?? 'Peserta' }}
                </p>
                <div style="display:flex;justify-content:space-between;align-items:center;font-size:10.5px;color:#9CA3B0;">
                  <span style="font-style:italic;">{{ Str::limit($nper->alasan_perpanjangan ?? 'Minta perpanjangan', 22) }}</span>
