<div style="position:relative;" wire:poll.3s wire:click.outside="closeDropdown">
    ...
    <div id="notif-drop" class="{{ $isOpen ? '' : 'hidden' }}" style="position:absolute;right:0;top:46px;width:320px;
      ...
      <div style="max-height:340px;overflow-y:auto;">
        ...
        {{-- List Pesan Masuk (Hubungi Kami) --}}
        @foreach($notifPesan as $npesan)
        <a href="{{ route('admin.pesan.show', $npesan) }}"
           style="display:flex;align-items:flex-start;gap:10px;padding:12px 16px;text-decoration:none;border-bottom:1px solid #F0F1F5;background:#F0F9FF;transition:background .15s;"
           onmouseover="this.style.background='#E0F2FE'" onmouseout="this.style.background='#F0F9FF'">
          <div style="width:34px;height:34px;border-radius:9px;background:rgba(14,165,233,.18);display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px;">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#0284C7" stroke-width="2.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
          </div>
          <div style="flex:1;min-width:0;">
            <p style="margin:0 0 1px;font-size:12.5px;font-weight:800;color:#0369A1;line-height:1.3;">
              Pesan Masuk: {{ $npesan->nama }}
            </p>
            <p style="margin:0 0 3px;font-size:11.5px;color:#334155;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
              {{ \Illuminate\Support\Str::limit($npesan->pesan, 35) }}
            </p>
            <div style="display:flex;justify-content:space-between;align-items:center;font-size:10.5px;color:#64748B;">
              <span style="font-weight:700;color:#0284C7;">{{ $npesan->email }}</span>
              <span>{{ $npesan->created_at?->diffForHumans() }}</span>
            </div>
          </div>
        </a>
        @endforeach
        ...
        <div style="padding:28px 16px;text-align:center;color:#9CA3B0;">
          ...
          <p style="margin:2px 0 0;font-size:11px;color:#9CA3B0;">Semua transaksi &amp; pesan telah dibaca.</p>
        </div>
        @endif
      </div>

      @if($totalNotifCount > 0)
      <div style="padding:8px;background:#F9FAFB;border-top:1px solid #E2E4EB;display:flex;justify-content:space-between;align-items:center;padding:8px 14px;">
        <a href="{{ route('admin.pesan.index') }}"
           style="font-size:11px;font-weight:800;color:#0284C7;text-decoration:none;">
          Pesan Masuk ({{ $countPesan }}) &rarr;
        </a>
        <a href="{{ route('admin.pembayaran.index', ['status' => 'menunggu_verifikasi']) }}"
           style="font-size:11px;font-weight:800;color:#3B82F6;text-decoration:none;">
