...
<div style="padding:24px 32px;max-width:1100px;margin:0 auto;width:100%;">
    ...
    <div style="display:grid;grid-template-columns:1fr 340px;gap:24px;align-items:start;">
        ...
        <div>
            {{-- Form Tambah Materi --}}
            <div class="fcc-card" style="padding:24px;margin-bottom:24px;background:linear-gradient(to right, #FFF, #FAFAFC);border-left:4px solid #FFC81A;">
                <h3 style="font-size:15px;font-weight:800;color:#131218;margin:0 0 16px;display:flex;align-items:center;gap:8px;">
                    @include('components.icon',['name'=>'plus-circle','size'=>18,'style'=>'color:#FFC81A'])
                    Tambah Materi Baru
                </h3>
                
                <form method="POST" action="{{ route('admin.materi-pelatihan.store', $selectedPelatihan->id) }}" style="display:flex;gap:16px;align-items:flex-end;">
                    @csrf
                    <input type="hidden" name="urutan" value="{{ $selectedPelatihan->materi->count() + 1 }}">
                    
                    <div style="flex:2;">
                        <label style="font-size:11.5px;font-weight:700;color:#6B7280;display:block;margin-bottom:6px;">Judul Materi</label>
                        <input type="text" name="judul_materi" required placeholder="Contoh: Pengenalan Dasar HTML" class="fcc-input" style="padding:12px 16px;font-size:13.5px;width:100%;border-radius:10px;">
                    </div>
                    
                    <div style="flex:1;">
                        <label style="font-size:11.5px;font-weight:700;color:#6B7280;display:block;margin-bottom:6px;">Jam Pelajaran (JP)</label>
                        <input type="number" name="jam_pelajaran" min="1" required placeholder="Contoh: 2" class="fcc-input" style="padding:12px 16px;font-size:13.5px;width:100%;border-radius:10px;">
                    </div>

                    <button type="submit" class="fcc-btn-gold" style="padding:12px 24px;font-size:13.5px;font-weight:800;border:none;border-radius:10px;cursor:pointer;height:43px;display:flex;align-items:center;gap:8px;">
                        @include('components.icon',['name'=>'check','size'=>16]) Simpan
                    </button>
                </form>
            </div>
            ...
            <div class="fcc-card" style="padding:0;overflow:hidden;">
                <div style="padding:18px 24px;border-bottom:1px solid #F0F1F5;display:flex;justify-content:space-between;align-items:center;background:#FDFDFE;">
                    ...
                    <span style="font-size:12px;font-weight:700;color:#FFC81A;background:rgba(255,200,26,.15);padding:4px 10px;border-radius:20px;">
                            ...
                            <tr style="border-bottom:1px solid #F0F1F5;transition:background .2s;" onmouseover="this.style.background='#F8F9FB'" onmouseout="this.style.background='none'">
                                ...
                                <td style="padding:16px 24px;text-align:center;">
                                    <form action="{{ route('admin.materi-pelatihan.destroy', [$selectedPelatihan->id, $mat->id]) }}" method="POST" style="margin:0;" onsubmit="return confirm('Yakin ingin menghapus materi ini secara permanen?');">
                                        @csrf @method('DELETE')
                                        <button type="submit" style="background:#FFF;border:1px solid #FEE2E2;color:#EF4444;width:34px;height:34px;border-radius:10px;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all .2s;margin:0 auto;box-shadow:0 2px 4px rgba(239,68,68,.05);" onmouseover="this.style.background='#FEF2F2';this.style.borderColor='#FCA5A5';this.style.transform='translateY(-1px)'" onmouseout="this.style.background='#FFF';this.style.borderColor='#FEE2E2';this.style.transform='none'" title="Hapus Materi">
