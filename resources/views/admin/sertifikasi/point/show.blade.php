    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;">
        <div>
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px;">
                <h1 style="font-size:24px;font-weight:900;color:#131218;margin:0;letter-spacing:-0.5px;">Point Peserta Sertifikasi</h1>
                <span style="background:#F7F8FA;color:#6B7280;padding:4px 10px;border-radius:6px;font-size:12px;font-weight:700;">Uji Kompetensi</span>
            </div>
            <p style="color:#6B7280;margin:0;font-size:14px;display:flex;align-items:center;gap:6px;">
                <span style="font-weight:700;color:#131218;">Judul:</span> {{ $jadwal->sertifikasi->judul ?? '-' }} 
        ...
        <a href="{{ route('admin.sertifikasi.point.index') }}" class="fcc-btn-gold" style="padding:10px 20px;font-size:12.5px;font-weight:800;border:none;border-radius:10px;cursor:pointer;display:inline-block;text-decoration:none;background:#EF4444;color:#FFF;box-shadow:0 4px 12px rgba(239,68,68,.2);">
            KEMBALI
        </a>
    </div>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="5" style="text-align:center;padding:48px 24px;color:#9CA3B0;">
                            ...
                            <p style="font-size:12.5px;margin:0;">Tidak ada peserta yang terdaftar pada jadwal sertifikasi ini.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<div id="nilai-modal" style="display:none;position:fixed;inset:0;z-index:9998;background:rgba(19,18,24,.55);backdrop-filter:blur(4px);align-items:center;justify-content:center;">
    <div style="background:#FFF;border-radius:18px;padding:32px 28px;max-width:550px;width:90%;position:relative;box-shadow:0 24px 64px rgba(0,0,0,.18);max-height:90vh;overflow-y:auto;display:flex;flex-direction:column;">
        ...
        <div style="margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid #F0F1F5;">
            <h2 style="font-size:18px;font-weight:900;color:#0F0F14;margin:0 0 4px;">Input Nilai Peserta Sertifikasi</h2>
        ...
        <form id="nilai-form" method="POST" action="">
            ...
            @else
                <div style="background:#FEF2F2;border:1px solid #FCA5A5;padding:16px;border-radius:12px;margin-bottom:24px;text-align:center;">
                    <p style="margin:0;color:#EF4444;font-size:13px;font-weight:600;">Sertifikasi ini belum memiliki materi. Silakan tambahkan materi sertifikasi terlebih dahulu.</p>
                </div>
            @endif
            ...
            <div style="display:flex;justify-content:flex-end;gap:12px;">
                ...
                <button type="submit" class="fcc-btn-gold" style="padding:10px 24px;font-size:13px;font-weight:800;border:none;border-radius:10px;cursor:pointer;" {{ (!$jadwal->sertifikasi || !$jadwal->sertifikasi->materi || $jadwal->sertifikasi->materi->count() == 0) ? 'disabled' : '' }}>Simpan Nilai</button>
            </div>
        </form>
    </div>
</div>
