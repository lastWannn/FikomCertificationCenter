...
<div style="padding:24px;">
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;flex-wrap:wrap;gap:16px;">
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;">
      ...
      <a href="{{ route('admin.sertifikasi.edit', $sertifikasi) }}" style="display:inline-flex;align-items:center;gap:8px;padding:9.5px 16px;border-radius:30px;border:1.5px solid #131218;background:#131218;color:#FFC81A;font-size:13px;font-weight:800;text-decoration:none;transition:all .18s;">
        ...
        <div style="padding:12px 18px;border-top:1px solid #F0F1F5;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <div>
              ...
              <p style="margin:0;font-size:11.5px;color:#9CA3B0;">{{ $j->tgl_pelaksanaan->translatedFormat('d M Y') }} &bull; {{ substr($j->jam_mulai, 0, 5) }} – {{ substr($j->jam_selesai, 0, 5) }} &bull; Kuota: {{ $j->kuota_peserta }}</p>
              @else
              <p style="margin:0;font-size:13px;font-weight:700;color:#131218;">{{ $j->tgl_pelaksanaan->translatedFormat('d M Y') }}</p>
            ...
            <div style="display:flex;align-items:center;gap:6px;flex-wrap:nowrap;">
              @if($ks)
                ...
                <form action="{{ route('admin.jadwal-sertifikasi.nonaktifkan', $j) }}" method="POST" style="margin:0;display:inline-flex;align-items:center;">
                  @csrf
                ...
                <a href="{{ route('admin.kegiatan.show', $ks->kegiatan) }}" target="_blank" title="Lihat Halaman Publik Kegiatan"
              ...
              @else
                ...
                <form action="{{ route('admin.jadwal-sertifikasi.aktifkan', $j) }}" method="POST" style="margin:0;display:inline-flex;align-items:center;">
                  @csrf
              ...
              {{-- Edit Button --}}
              <a href="{{ route('admin.jadwal-sertifikasi.edit', $j) }}" title="Edit Jadwal"
...
<div id="jadwal-modal" style="display:none;position:fixed;inset:0;z-index:9998;background:rgba(19,18,24,0.65);backdrop-filter:blur(8px);align-items:center;justify-content:center;">
    <div style="background:#FFFFFF;border:2px solid #131218;border-radius:24px;padding:32px;max-width:640px;width:92%;position:relative;box-shadow:0 24px 60px rgba(0,0,0,0.3);max-height:90vh;overflow-y:auto;display:flex;flex-direction:column;">
        ...
        <form action="{{ route('admin.jadwal-sertifikasi.store', $sertifikasi) }}" method="POST">
            @csrf
