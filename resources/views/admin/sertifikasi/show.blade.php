        ...
        <div>
            ...
            <div class="fcc-card" style="padding:20px;margin-bottom:14px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
                    ...
                    <button onclick="openMateriModal()" style="font-size:11px;color:#FFC81A;font-weight:700;text-decoration:none;background:none;border:none;cursor:pointer;padding:0;">+ Tambah</button>
                </div>
                ...
                <div style="display:flex;align-items:center;gap:12px;padding:8px 0;border-top:1px solid #F0F1F3;" class="tbl-row">
                    ...
                    <div style="display:flex;gap:6px;flex-shrink:0;">
                        <button onclick="openEditMateriModal({{ $m->id }}, '{{ addslashes($m->judul_materi) }}', {{ $m->urutan }})" title="Edit" style="background:none;border:none;cursor:pointer;color:#9CA3B0;display:flex;padding:4px;transition:color .18s;"
                           onmouseover="this.style.color='#FFC81A'" onmouseout="this.style.color='#9CA3B0'">
                            @include('components.icon',['name'=>'edit','size'=>14])
                        </button>
                        <button onclick="confirmMateriDelete('{{ route('admin.materi-sertifikasi.destroy',[$sertifikasi->id,$m->id]) }}', '{{ addslashes($m->judul_materi) }}')" style="background:none;border:none;cursor:pointer;color:#9CA3B0;display:flex;padding:4px;transition:color .18s;"
    ...
    <div style="background:#1C1B22;border:1px solid rgba(255,255,255,.1);border-radius:20px;padding:32px;max-width:500px;width:90%;box-shadow:0 24px 60px rgba(0,0,0,.5);animation:modalIn .25s ease;margin:auto;position:relative;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
            <div>
                ...
                <p style="color:rgba(255,255,255,.4);font-size:13px;margin:0;">Masukkan detail materi sertifikasi.</p>
            </div>
        ...
        <form id="materi-form" method="POST" enctype="multipart/form-data" style="margin:0;">
            ...
            <div style="display:grid;gap:20px;margin-bottom:24px;">
                <div>
                    ...
                    <input type="text" name="judul_materi" id="f-judul" required placeholder="Contoh: Modul 1" style="width:100%;background:rgba(255,255,255,.05);border:1.5px solid rgba(255,255,255,.1);border-radius:10px;padding:10px 14px;color:#FFF;font-size:14px;outline:none;transition:border-color .2s;box-sizing:border-box;" onfocus="this.style.borderColor='#FFC81A'" onblur="this.style.borderColor='rgba(255,255,255,.1)'">
                </div>
                <div>
...
<script>
const STORE_URL = '{{ route('admin.materi-sertifikasi.store', $sertifikasi->id) }}';
const UPDATE_URL_BASE = '{{ url("admin/materi-sertifikasi/{$sertifikasi->id}") }}';
...
function openMateriModal() {
    ...
    document.getElementById('f-urutan').value = '{{ $sertifikasi->materi->max("urutan") + 1 }}';
    document.getElementById('f-file').value = '';
    showModal('materi-modal');
}

function openEditMateriModal(id, judul, urutan) {
