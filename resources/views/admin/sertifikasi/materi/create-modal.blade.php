{{-- ── TAMBAH MATERI SERTIFIKASI MODAL ────────────────────────────────────── --}}
<div id="create-modal" style="display:{{ $errors->has('judul_materi') && !session('materi_id') ? 'flex' : 'none' }};position:fixed;inset:0;z-index:9998;background:rgba(19,18,24,.55);backdrop-filter:blur(4px);align-items:center;justify-content:center;">
    <div style="background:#FFF;border-radius:18px;padding:32px 28px;max-width:500px;width:90%;position:relative;box-shadow:0 24px 64px rgba(0,0,0,.18);display:flex;flex-direction:column;">
        ...
        <div style="margin-bottom:20px;">
            <h2 style="font-size:18px;font-weight:900;color:#0F0F14;margin:0 0 4px;">Tambah Materi Sertifikasi Baru</h2>
            <p style="color:#6B7280;font-size:13px;margin:0;">Isi informasi materi untuk sertifikasi ini.</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.materi-sertifikasi.store', $selectedSertifikasi) }}" enctype="multipart/form-data">
            ...
            <div style="margin-bottom:12px;">
                ...
                <input type="text" name="judul_materi" value="{{ old('judul_materi') }}" placeholder="Contoh: Unit 1 - Dasar Kompetensi" required class="fcc-input" style="padding:8px 12px;font-size:13px;">
            ...
            <div style="margin-bottom:24px;">
                <label style="font-size:11px;font-weight:700;color:#6B7280;display:block;margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px;">DESKRIPSI <span style="font-weight:500;text-transform:none;color:#9CA3B0;">(Opsional)</span></label>
                <textarea name="isi" rows="3" placeholder="Penjelasan singkat materi sertifikasi..." class="fcc-input" style="padding:8px 12px;font-size:13px;resize:vertical;">{{ old('isi') }}</textarea>
            </div>
