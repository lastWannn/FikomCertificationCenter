    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:24px;">
        <div>
            <h1 style="font-size:24px;font-weight:900;color:#131218;margin:0 0 6px;letter-spacing:-0.5px;">
                Kelola <span style="color:#FFC81A;">Materi Sertifikasi</span>
            </h1>
            <p style="color:#6B7280;font-size:14px;margin:0;">Pilih program sertifikasi dan kelola materi uji kompetensinya di sini.</p>
        </div>
    </div>
    ...
    <div class="fcc-card" style="margin-bottom:24px;padding:24px;background:#FFF;border-radius:16px;box-shadow:0 4px 20px rgba(0,0,0,0.03);">
        <form id="select-sertifikasi-form" method="GET" action="{{ route('admin.sertifikasi.materi.index') }}">
            ...
            <div style="position:relative;">
                ...
                <select name="sertifikasi_id" onchange="document.getElementById('select-sertifikasi-form').submit()" class="fcc-input" style="padding:14px 16px 14px 44px;font-size:14.5px;font-weight:700;color:#131218;width:100%;background:#F7F8FA;border:1.5px solid #E2E4EB;border-radius:12px;cursor:pointer;appearance:none;transition:all .2s;" onmouseover="this.style.borderColor='#FFC81A';this.style.background='#FFF'" onmouseout="this.style.borderColor='#E2E4EB';this.style.background='#F7F8FA'">
                    <option value="" style="color:#9CA3B0;">-- Silahkan Pilih Program Sertifikasi --</option>
                    @foreach($sertifikasis as $s)
                        <option value="{{ $s->id }}" {{ ($selectedSertifikasi && $selectedSertifikasi->id == $s->id) ? 'selected' : '' }}>
                            {{ $s->kode }} - {{ $s->judul }}
                        </option>
                    @endforeach
                </select>
                    ...
                    <table style="width:100%;border-collapse:collapse;font-size:13.5px;">
                        <thead>
                            <tr style="background:#F7F8FA;border-bottom:1px solid #E2E4EB;">
                                ...
                                <th style="text-align:left;padding:14px 24px;font-weight:800;color:#9CA3B0;text-transform:uppercase;font-size:11px;letter-spacing:0.5px;">Deskripsi</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td colspan="4" style="text-align:center;padding:48px 24px;color:#9CA3B0;">
                                    ...
                                    <p style="font-weight:700;color:#6B7280;margin:0 0 4px;font-size:14px;">Belum Ada Materi Sertifikasi</p>
                                    <p style="font-size:12.5px;margin:0;">Silakan tambahkan materi sertifikasi pertama melalui tombol di atas.</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
        ...
        <div>
            <div class="fcc-card" style="padding:20px;position:sticky;top:24px;">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <p style="margin:0;font-size:14px;font-weight:800;color:#FFC81A;">{{ $selectedSertifikasi->kategori->nama_kategori ?? 'Tidak ada kategori' }}</p>
                </div>

                <div style="margin-bottom:20px;">
                    <p style="margin:0 0 4px;font-size:11.5px;font-weight:700;color:#6B7280;">Kode Sertifikasi</p>
                    <p style="margin:0;font-size:13.5px;font-weight:700;color:#131218;font-family:monospace;background:#F7F8FA;padding:6px 10px;border-radius:8px;display:inline-block;border:1px solid #E2E4EB;">{{ $selectedSertifikasi->kode }}</p>
                </div>

                <p style="font-size:11.5px;font-weight:700;color:#6B7280;margin:0 0 8px;">Poster Sertifikasi</p>
                @if($selectedSertifikasi->gambar)
                    <img src="{{ Storage::url($selectedSertifikasi->gambar) }}" alt="Poster" style="width:100%;border-radius:12px;object-fit:cover;aspect-ratio:3/4;box-shadow:0 8px 24px rgba(0,0,0,0.12);border:1px solid #E2E4EB;">
                @else
