{{-- ── TAMBAH MATERI MODAL (Neo-Brutalist Glassmorphism) ────────────────────────────────────── --}}
<div id="create-modal" style="display:{{ $errors->has('judul_materi') && !session('materi_id') ? 'flex' : 'none' }};position:fixed;inset:0;z-index:9998;background:rgba(19,18,24,0.65);backdrop-filter:blur(8px);align-items:center;justify-content:center;" onclick="if(event.target===this) this.style.display='none'">
    <div style="background:#FFFFFF;border:2px solid #131218;border-radius:24px;padding:32px;max-width:520px;width:92%;position:relative;box-shadow:0 24px 60px rgba(0,0,0,0.3);display:flex;flex-direction:column;" onclick="event.stopPropagation()">
        ...
        <div style="margin-bottom:20px;border-bottom:2px solid #E5E7EB;padding-bottom:14px;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
                ...
                <h2 style="font-size:19px;font-weight:900;color:#131218;margin:0;">Tambah Modul Materi Baru</h2>
            </div>
            <p style="color:#64748B;font-size:12.5px;margin:0;font-weight:500;">Tambahkan topik modul materi baru untuk {{ $selectedPelatihan->judul }}.</p>
        </div>

        <form method="POST" action="{{ route('admin.materi-pelatihan.store', $selectedPelatihan) }}">
            @csrf
            <input type="hidden" name="urutan" value="{{ $selectedPelatihan->materi->count() + 1 }}">
            
            {{-- JUDUL MATERI --}}
            <div style="margin-bottom:14px;">
                <label style="font-size:11px;font-weight:800;color:#131218;display:block;margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px;">Judul Materi / Topik <span style="color:#EF4444;">*</span></label>
                <input type="text" name="judul_materi" value="{{ old('judul_materi') }}" placeholder="Contoh: Pengenalan Dasar HTML & CSS" required class="fcc-input" style="padding:9.5px 14px;font-size:13.5px;width:100%;border:1.5px solid #CBD5E1;border-radius:10px;">
            ...
            <div style="margin-bottom:24px;">
                <label style="font-size:11px;font-weight:800;color:#131218;display:block;margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px;">Durasi Jam Pelajaran (JP) <span style="color:#EF4444;">*</span></label>
                <input type="number" name="jam_pelajaran" value="{{ old('jam_pelajaran', 1) }}" min="1" required placeholder="1" class="fcc-input" style="padding:9.5px 14px;font-size:13.5px;width:100%;border:1.5px solid #CBD5E1;border-radius:10px;">
            ...
            <div style="display:flex;justify-content:flex-end;gap:12px;margin-top:10px;">
                ...
                <button type="submit"
                    ...
                    @include('components.icon',['name'=>'check','size'=>16]) Simpan Modul Materi
                </button>
            </div>
        </form>
    </div>
