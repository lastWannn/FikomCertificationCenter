...
<div style="padding-top:64px; background:#131218; min-height: calc(100vh - 64px);">
    {{-- Hero Header --}}
    <div style="background:#131218; padding:48px 24px 44px; text-align:center; position:relative; overflow:hidden; border-bottom:1px solid #1E1D26;">
        <!-- Ambient Glow -->
        <div style="position:absolute; top:-50%; left:50%; transform:translateX(-50%); width:600px; height:600px; border-radius:50%; background:radial-gradient(circle, rgba(255,200,26,0.08), transparent 70%); pointer-events:none;"></div>
        ...
        <div style="position:relative; z-index:1; max-width:800px; margin:0 auto;">
            ...
            <h1 style="color:#FFFFFF; font-size:clamp(28px,4.5vw,42px); font-weight:900; margin:0 0 12px; letter-spacing:-0.8px; line-height:1.15;">
                Tata Cara <span style="color:#FFC81A; background:#131218; padding:2px 10px; border-radius:6px;">Pendaftaran</span>
            </h1>
            <p style="color:rgba(255,255,255,0.8); font-size:15px; margin:0 auto; max-width:480px; line-height:1.6; font-weight:500;">
                Panduan praktis mendaftar kegiatan pelatihan &amp; sertifikasi FCC FIKOM UMI dalam 4 langkah mudah.
            </p>
        </div>
    </div>

    {{-- Main Section — Clean White Section (Matching Home Page) --}}
    <div style="background:#FFFFFF; padding:80px 24px 90px; border-bottom:1px solid #E5E7EB;">
        <div style="max-width:1100px; margin:0 auto; position:relative; z-index:1;">
            ...
            <div style="position:relative; margin-bottom:44px;">
                ...
                <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:16px; position:relative; z-index:1;">
                    ...
                    <div id="step-wrapper-{{ $si }}" style="text-align:center; cursor:pointer; padding:6px;" onclick="setStepInline({{ $si }})" onmouseenter="clearInterval(stepTimer); setStepInline({{ $si }})" onmouseleave="startTimer()">
            ...
            <div style="text-align:center;">
                <a href="{{ route('auth.register') }}" style="padding:14px 32px; font-size:15px; font-weight:800; background:#FFC81A; color:#131218; border:1.5px solid #131218; border-radius:30px; text-decoration:none; display:inline-flex; align-items:center; gap:10px; box-shadow:0 6px 20px rgba(255,200,26,0.35); transition:all 0.2s ease;"
...
<script>
    ...
    function setStepInline(s) {
        ...
        for (let i = 0; i < STEP_COUNT; i++) {
            ...
            if (ic) ic.style.color = isActive ? '#131218' : (isPast ? '#131218' : '#6B7280');
