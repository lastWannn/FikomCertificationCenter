...
<div style="padding-top:68px; background:linear-gradient(180deg, #131218 0%, #0e0d14 120px, #0e0d14 100%); min-height: 100vh;">
    ...
    <div style="padding:52px 24px;max-width:1000px;margin:0 auto;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;">
            {{-- Form --}}
            <div>
                <h2 style="font-size:20px;font-weight:800;color:#FFF;margin:0 0 20px;">Kirim Pesan</h2>
                <form action="{{ route('landing.kontak.post') }}" method="POST">
                    ...
                    <div style="margin-bottom:14px;">
                        <label style="display:block;font-size:11px;font-weight:700;color:rgba(255,255,255,.6);margin-bottom:5px;text-transform:uppercase;letter-spacing:.7px;">{{ $l }} *</label>
                        <div style="position:relative;">
                            @include('components.icon',['name'=>$ic,'size'=>14,'style'=>'position:absolute;left:13px;top:50%;transform:translateY(-50%);color:rgba(255,255,255,.4);pointer-events:none;'])
                            <input type="{{ $t }}" name="{{ $n }}" value="{{ old($n) }}" placeholder="{{ $p }}" required class="fcc-input-dark" style="padding-left:38px;width:100%;box-sizing:border-box;"
                    ...
                    <div style="margin-bottom:20px;">
                        <label style="display:block;font-size:11px;font-weight:700;color:rgba(255,255,255,.6);margin-bottom:5px;text-transform:uppercase;letter-spacing:.7px;">Pesan *</label>
                        <textarea name="pesan" rows="5" required placeholder="Tuliskan pertanyaan atau pesanmu…" class="fcc-input-dark" style="resize:vertical;width:100%;box-sizing:border-box;">{{ old('pesan') }}</textarea>
                        @error('pesan')<p style="color:#EF4444;font-size:12px;margin:4px 0 0;">{{ $message }}</p>@enderror
                    </div>
                    <button type="submit" class="fcc-btn-gold btn-shine" style="width:100%;justify-content:center;padding:12px;font-size:15px;border-radius:12px;">
            ...
            {{-- Info --}}
            <div>
                <h2 style="font-size:20px;font-weight:800;color:#FFF;margin:0 0 20px;">Informasi Kontak</h2>
                <div style="display:flex;flex-direction:column;gap:18px;margin-bottom:24px;">
                    ...
                    <div style="display:flex;gap:14px;align-items:flex-start;">
                        <div style="width:40px;height:40px;border-radius:11px;flex-shrink:0;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);display:flex;align-items:center;justify-content:center;">
                        ...
                        <div>
                            ...
                            <p style="margin:0;color:#FFF;font-size:14px;">{{ $val }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                @else
                <div style="background:rgba(255,255,255,.03);border:1.5px solid rgba(255,255,255,.08);border-radius:12px;padding:28px;text-align:center;height:200px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;">
                    @include('components.icon',['name'=>'map-pin','size'=>32,'style'=>'color:rgba(255,200,26,.35)'])
                    <p style="color:rgba(255,255,255,.4);font-size:13px;margin:0;">Kampus UMI Makassar</p>
                </div>
                @endif
            </div>
        </div>
